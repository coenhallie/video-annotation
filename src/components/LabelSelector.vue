<template>
  <div class="label-selector">
    ...
    <div v-if="!readonly" class="space-y-3">
      ...
      <div
        v-if="quickLabels.length > 0 && !showDropdown"
      ...
      <!-- Searchable Dropdown for All Labels -->
      <div class="relative">
        <div
          @click="toggleDropdown"
          class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm cursor-text focus-within:ring-1 focus-within:ring-blue-500 focus-within:border-blue-500"
          :class="{ 'rounded-b-none border-b-0': showDropdown }"
        >
            ...
            <input
              ...
              :placeholder="
                selectedLabels.length > 0
                  ? `${selectedLabels.length} label(s) selected - Search or add more...`
                  : 'Search and select labels...'
              "
              class="block w-full pl-10 pr-10 py-2 text-sm bg-transparent border-0 focus:outline-none focus:ring-0 placeholder-gray-500"
            />
            <button
              @click="toggleDropdown"
              type="button"
              class="absolute inset-y-0 right-0 flex items-center pr-2 hover:bg-gray-50 rounded-r-md"
            >
              <svg
...
const emit = defineEmits(['update:modelValue', 'manage-labels']);
...
// Quick labels (most commonly used default labels)
const quickLabels = computed(
  () =>
    labels.value
      .filter((label) => label.isDefault && label.isActive)
      .slice(0, 4) // Show first 4 default labels as quick options
);
...
const toggleLabel = (labelId: string) => {
  ...
  if (index > -1) {
    ...
    // Add label (check max limit)
    if (props.maxLabels && selectedLabels.value.length >= props.maxLabels) {
...
// New methods for enhanced dropdown functionality
...
watch(showDropdown, (isOpen) => {
  if (!isOpen) {
    // Don't clear search immediately to allow for selection
    setTimeout(() => {
      if (!showDropdown.value) {
        searchQuery.value = '';
        highlightedIndex.value = -1;
