<template>
  <!-- One line: attribution left, status right. No "QA" heading - the five
       values name themselves, and the select is the only control in its block.
       That leaves attribution and status as a label/value pair, the same shape
       the Watched and Annotations blocks beside it already use.

       items-center rather than items-baseline: the attribution truncates, and a
       box with hidden overflow has its baseline synthesized from the border
       box, which would drop it out of line with the select. -->
  <div class="flex items-center gap-2">
    <!-- Who last touched it. A status with no author is unattributable in a
         tool several people share. min-w-0 so a long name truncates here
         instead of pushing the status out of its column. -->
    <p
      v-if="attribution"
      data-testid="qa-status-attribution"
      class="min-w-0 truncate font-mono text-[10px] tracking-wider text-gray-500 dark:text-gray-400"
    >
      {{ attribution }}
    </p>
    ...
    <!-- A native select, the same choice VideoControls makes for playback
         ...
         -mr-[5px] cancels the right padding and border optically. The box
         keeps them, so the hover outline never sits on the glyphs, but the
         status is now the value in a label/value row and has to land on the
         same right-hand column as the Watched percentage above it - which the
         5px inside an invisible border would otherwise hold it short of. -->
    <select
      ...
      :class="[
        'ml-auto -mr-[5px] shrink-0 cursor-pointer appearance-none rounded border border-transparent bg-transparent py-0.5 pl-1 pr-1 font-mono text-[10px] tracking-wider transition-colors',
